# Learn To Debate chat tool - LDCT - CLIENT SIDE - INTERNET
# Viktorija Sokolova MSc IT Birkbeck 2023
# -------------------------------
...
# Frame 2 - Chat
# Here user provides a username in the entry box.
# Once username is provided, the client connects to the server that is already running.
...
# Connection details
HOST =  "90.203.88.14" # IPV4 public address of the server on author's device.
PORT = 12345
...
def add_widgets_frame_two():
    ...
    # Will only connect if username is provided.
    # If empty - will show error until username is entered.
    # Once connection established, username and connect buttons are disabled.
    # Function also starts a thread - listening_to_server.
    # User is also provided with information how to end chat.
    # If connection successfuly, End Chat button is enabled.
    def join_chat():
        ...
        if username != "":
            ...
            chat_box.config(state=tk.NORMAL)
            chat_box.insert("1.0","<CHAT>: Username incorrect. Try again." + '\n')
            chat_box.config(state=tk.DISABLED)
    ...
    username_entry = ctk.CTkEntry(inner_frame_top,
            validate="focusout", width=199, height=7,
...
# Function creates a temporary list, iterates over text file, adds to the list lines that don't repeat, and corrects the file by overwriting it.
    ...
    # Back button left for testing. I allows to go back to second frame from third frame.
